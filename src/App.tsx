import React, { useState, useEffect, useCallback } from 'react';
    import { Search, MapPin, Wind, Droplets, Sun, Cloud, CloudRain, CloudSnow, AlertTriangle } from 'lucide-react';
    ...
    export default function App() {
      ...
      const fetchWeatherData = useCallback(async (lat: number, lon: number) => {
        ...
        try {
          ...
          if (!response.ok) {
            throw new Error('Weather data not found. Check your API key and location.');
          }
      ...
      const getLocation = useCallback(() => {
        ...
        if (navigator.geolocation) {
          navigator.geolocation.getCurrentPosition(async (position) => {
            ...
            setError("Geolocation permission denied. Falling back to default location.");
            handleSearch();
          });
        } else {
          setError("Geolocation is not supported by this browser. Falling back to default location.");
          handleSearch();
        }
      ...
      if (!API_KEY) {
        return (
          <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 text-white p-8 flex items-center justify-center">
            <div className="w-full max-w-md bg-black bg-opacity-50 backdrop-blur-lg rounded-2xl shadow-2xl p-8 text-center">
                <AlertTriangle className="mx-auto h-16 w-16 text-yellow-400 mb-4" />
                <h1 className="text-2xl font-bold mb-2">Configuration Error</h1>
                <p className="text-gray-300">
                    The OpenWeatherMap API key is missing. Please create a <code className="bg-gray-700 p-1 rounded">.env.local</code> file in the root of the project and add your API key as follows:
                </p>
                <pre className="bg-gray-800 text-left p-4 rounded-lg mt-4 text-sm text-yellow-300">
